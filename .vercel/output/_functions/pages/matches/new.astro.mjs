/* empty css                                           */
import { e as createComponent, r as renderTemplate, n as defineScriptVars, k as renderComponent, m as maybeRenderHead } from '../../chunks/astro/server_Ctxpu_nC.mjs';
import 'piccolore';
...
const $$New = createComponent(async ($$result, $$props, $$slots) => {
  ...
  // Map de jugadores por ID para obtener su civilizaci\xF3n
  const playersMap = {};
  players.forEach(p => {
    playersMap[p.id] = p;
  });
  ...
  function updateCivDisplay(select) {
    ...
    if (selectedOption && selectedOption.value) {
      ...
      if (player && player.favoriteCiv) {
        display.textContent = \`Civilizaci\xF3n: \${player.favoriteCiv}\`;
    ...
    if (matchType === '1v1') {
      html = \`
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="aoe-card">
            <h3 class="aoe-title text-lg mb-4 text-green-400">\u{1F3C6} Ganador</h3>
            \${createPlayerSelect(1, 1, 'Jugador')}
          </div>
          <div class="aoe-card">
            <h3 class="aoe-title text-lg mb-4 text-red-400">\u{1F480} Perdedor</h3>
      ...
      html = \`
        ...
        <div class="aoe-card mb-6">
          <h3 class="aoe-title text-lg mb-4">\u{1F3B2} Aleatorizar Equipos</h3>
          <div class="mb-4">
          ...
          <button type="button" id="randomizeBtn" class="aoe-btn w-full">
            \u{1F3B2} Generar Equipos al Azar
          </button>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div class="aoe-card">
            <h3 class="aoe-title text-lg mb-4 text-green-400">\u{1F3C6} Equipo Ganador</h3>
            <div class="space-y-4">\${team1Players}</div>
          </div>
          <div class="aoe-card">
            <h3 class="aoe-title text-lg mb-4 text-red-400">\u{1F480} Equipo Perdedor</h3>
      ...
      html = \`
        ...
        <div class="aoe-card">
          <h3 class="aoe-title text-lg mb-4 text-red-400">\u{1F480} Perdedores</h3>
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    if (new Set(playerIds).size !== playerIds.length) {
      errorDiv.textContent = 'No puedes seleccionar el mismo jugador m\xE1s de una vez';
  ...
  // Frases argentinas para el meme (versi\xF3n picante)
  const frasesArgentinas = [
    "A CASA PETE",
    "SOS UN DESASTRE HERMANO",
    "TERRIBLE BOLUDO",
    "QU\xC9 HIJO DE PUTA MALO",
    "DEJ\xC1 DE HACER EL RID\xCDCULO",
    "TOMATEL\xC1 PELOTUDO",
    "SOS M\xC1S MALO QUE PATEAR UN PERRO",
    "TE COGIERON DE PARADO",
    "TE ROMPIERON EL ORTO",
    "AND\xC1 A CAGAR FLACO",
    "LA CONCHA DE TU MADRE",
    "QU\xC9 VERG\xDCENZA PAP\xC1",
    "NI EN PEDO VOLV\xC9S A GANAR",
    "CHUPAME LA PIJA",
    "QU\xC9 MAL QUE JUG\xC1S DIOS M\xCDO",
    "SOS UN QUESO TERRIBLE",
    "TE GARCHARON AMIGO",
    "SACATE LA CAMISETA PETE",
    "VOLV\xC9 A LA B MOG\xD3LICO",
    "AND\xC1 A JUGAR A LA PLAY NENE",
  ];

  const subFrases = [
    "Sos un desastre con patas",
    "Ni tu vieja te banca jugando",
    "La pr\xF3xima jug\xE1 al solitario mejor",
    "Tu civilizaci\xF3n te repudia",
    "Hasta un pibe de 5 a\xF1os te gana",
    "Dej\xE1 el mouse y and\xE1 a terapia",
    "Sos m\xE1s manco que el manco de lepanto",
    "Met\xE9 F10 F10 y dej\xE1 de joder",
    "Sos tan malo que das ternura",
    "Tu teclado se quiere suicidar",
    "Ni con walls te salv\xE1s crack",
  ...
  function generateMeme(losers, winners) {
    ...
    // Emoji de burla
    ctx.font = '80px Arial';
    ctx.fillText('\u{1F602}\u{1F480}\u{1F921}', canvas.width / 2, 300);
...
})();<\/script>`])), renderComponent($$result, "Layout", $$Layout, { "title": "Nueva Partida" }, { "default": async ($$result2) => renderTemplate` ${maybeRenderHead()}<div class="max-w-3xl mx-auto space-y-8"> <div class="text-center"> <h1 class="aoe-title text-3xl mb-2">Registrar Partida</h1> <p class="text-aoe-cream-dark">Selecciona el tipo de partida y los participantes</p> </div> ${players.length < 2 ? renderTemplate`<div class="aoe-panel text-center py-8"> <span class="text-5xl mb-4 block">⚠️</span> <h2 class="aoe-title text-xl mb-2">Jugadores insuficientes</h2> <p class="text-aoe-cream-dark mb-4">
Necesitas al menos 2 jugadores para registrar una partida.
</p> <a href="/players" class="aoe-btn-primary inline-block">
Agregar Jugadores
</a> </div>` : renderTemplate`<div class="aoe-panel"> <form id="matchForm" class="space-y-6"> <!-- Match Type --> <div> <label class="block text-sm font-semibold text-aoe-cream mb-3">
Tipo de Partida
</label> <div class="grid grid-cols-2 md:grid-cols-4 gap-3"> <label class="cursor-pointer"> <input type="radio" name="matchType" value="1v1" class="peer hidden" checked> <div class="aoe-card text-center peer-checked:border-aoe-gold-light peer-checked:shadow-gold transition-all"> <span class="text-2xl block mb-1">⚔️</span> <span class="text-sm">1v1</span> </div> </label> <label class="cursor-pointer"> <input type="radio" name="matchType" value="2v2" class="peer hidden"> <div class="aoe-card text-center peer-checked:border-aoe-gold-light peer-checked:shadow-gold transition-all"> <span class="text-2xl block mb-1">⚔️⚔️</span> <span class="text-sm">2v2</span> </div> </label> <label class="cursor-pointer"> <input type="radio" name="matchType" value="3v3" class="peer hidden"> <div class="aoe-card text-center peer-checked:border-aoe-gold-light peer-checked:shadow-gold transition-all"> <span class="text-2xl block mb-1">⚔️⚔️⚔️</span> <span class="text-sm">3v3</span> </div> </label> <label class="cursor-pointer"> <input type="radio" name="matchType" value="ffa" class="peer hidden"> <div class="aoe-card text-center peer-checked:border-aoe-gold-light peer-checked:shadow-gold transition-all"> <span class="text-2xl block mb-1">👑</span> <span class="text-sm">FFA</span> </div> </label> </div> </div> <!-- Players Container --> <div id="playersContainer" class="space-y-6"> <!-- Team 1 / Players will be inserted here by JS --> </div> <div id="formError" class="hidden text-red-400 text-sm text-center p-2 bg-red-900/20 rounded"></div> <button type="submit" class="aoe-btn-primary w-full">
Registrar Partida
</button> </form> </div>`} </div>  <div id="resultModal" class="fixed inset-0 z-50 hidden flex items-center justify-center p-4 backdrop-blur-md bg-black/90"> <div class="text-center space-y-4 max-w-4xl w-full"> <canvas id="memeCanvas" class="mx-auto border-4 border-aoe-gold rounded-lg shadow-[0_0_50px_rgba(218,165,32,0.5)]" width="800" height="500"></canvas> <div class="flex gap-4 justify-center"> <button id="downloadBtn" class="aoe-btn-primary text-lg px-8 py-3">
Descargar Imagen
</button> <button id="closeModalBtn" class="aoe-btn text-lg px-8 py-3">
Ver Partidas
</button> </div> </div> </div> ` }), defineScriptVars({ players }));
}, "C:/Users/tronc/OneDrive/Escritorio/proyects/proyect-two/age-boys/src/pages/matches/new.astro", void 0);

const $$file = "C:/Users/tronc/OneDrive/Escritorio/proyects/proyect-two/age-boys/src/pages/matches/new.astro";
